...
func (s *ProcessorService) ProcessLocation(ctx context.Context, lcn *domain.Location) error {
	...
	if s.checkDistance(lcn, lastLcn) {
		fmt.Println("Distance check passed, updating storage...")
		...
		fmt.Println("Distance check failed (too close)")
	}
